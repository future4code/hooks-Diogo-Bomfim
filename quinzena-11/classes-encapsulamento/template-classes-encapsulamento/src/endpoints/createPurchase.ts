...
export const createPurchase = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // COM TYPE
        //const product: Product = { id: findProduct[0].id, name: findProduct[0].name, price: findProduct[0].price }

        // COM CLASSE
        const newPurchase = new Purchase(Date.now().toString(), userId, productId, quantity, product.price * quantity)

        //COM TYPE
        //const newPurchase: Purchase = { id: Date.now().toString(), userId, productId, quantity, totalPrice: product.price * quantity }
